...
model = MultiOutputRegressor(estimator=ada_boost, n_jobs=-1)

estimators = np.arange(10, 1000, 10)
scores = dict()
current_index = 0
for n in estimators:
    model.set_params(estimator=AdaBoostRegressor(n_estimators=n))
...
for i in range(0, 5):
    ...
    model.set_params(estimator=AdaBoostRegressor(n_estimators=estimators[index]))
    model.fit(x_train, y_train)
